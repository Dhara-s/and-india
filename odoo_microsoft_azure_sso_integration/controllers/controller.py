...
class OAuth_Controller(http.Controller):
    ...
    def microsoft_signin(self, **kw):
        ...
        root_url = request.env['ir.config_parameter'].sudo().get_param(
            'web.base.url') + '/'
        oauth_provider_rec =\
            pool['ir.model.data'].sudo().get_object_reference('odoo_microsoft_azure_sso_integration','provider_microsoft')[1]
        provider = \
...
class OAuth_Login(Home):
    ...
    def list_providers(self):
        ...
        provider_microsoft = request.env.ref(
            'odoo_microsoft_azure_sso_integration.provider_microsoft')
